...
startPnt = Zmin;
newArray = zeros(size(chicken,1),3);%array after deleting points
trex = sortrows(chicken,3);
%generate a sliced point cloud
for i = 1 : n 
    count = 1;
    test = find(chicken(:,3)>startPnt);
    test2 = find(chicken(:,3)<startPnt+thickness);
    test = intersect(test,test2);
    tmp = zeros(size(test,1),3);
    for j = 1:size(test)
        tmp(j,:) = chicken(test(j),:);
    end
...
csvwrite('chicken_slice_before.dat',chicken);
csvwrite('chicken_slice_after.dat',newArray);

before = dir('chicken_slice_before.dat');
after = dir('chicken_slice_after.dat'); 

compression_ratio_slice_chicken = after.bytes / before.bytes;